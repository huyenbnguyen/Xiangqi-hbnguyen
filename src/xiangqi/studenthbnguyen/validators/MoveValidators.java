...
public class MoveValidators {
	public static MoveValidator<XiangqiState, XNC, MoveResult> isCorrectColor = (state, from, to) -> {
	...
	public static MoveValidator<XiangqiState, XNC, MoveResult> generalNotInCheck = (state, from, to) -> {
    	...
    	for (Entry<XNC, XiangqiPieceImpl> entry : gameCopy.getState().board.boardMap.entrySet()) {
    		if (entry.getValue().getColor() == color && 
    			...
    			state.moveMessage = "ILLEGAL: Move puts the general in check";
    			return ILLEGAL;
    		}  
    	}
